<?php
    ...
    if(isset($_POST['assign']) || isset($_POST['p_assign'])){
    $user_id= $_SESSION['u_id'];
    $id=$_GET['id'];
   
    if(empty($errors)){
       if(($_POST['name']=="") || ($_POST['gender']=="") || ($_POST['problem']=="")
        || ($_POST['age']=="") || ($_POST['shift']=="") || ($_POST['phone']=="") || ($_POST['address']=="") 
         || ($_POST['inputdate']=="")){
            ...
            if(isset($_POST['assign'])){
                ...
                $_SESSION['appointment_shift']=$shift;
                $_SESSION['appointment_doctorid']=$id;
                $_SESSION['appointment_time']=$appointment_time;
             echo '<script>confirm("Registration Fee: Rs 100\nPay With Khalti");</script>';
             echo '<script>window.location = "khaltipay.php"</script>';
            
               } else if(isset($_POST['p_assign'])){
            ...
            ('$name','$gender','$problem','$shift','$age','$phone','$address','$inputdate','$appointment_time','$status','$user_id','$id')";
           if($conn-> query($sql) == TRUE){
            echo '<script>confirm("Appointment Scheduled.. Go to Next page?");</script>';
...
?>
        ...
        <br>
        <hr>
        <p class="pay_option mb-4"> Take Appointment : </p>
   
        <div class="float-left">   
            <button type="submit" class="btn btn-success" name="assign">Pay Now</button>  
            <button type="submit" class="btn btn-primary" name="p_assign">Pay Later</button>  
